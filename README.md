...
以下样张均来自不同的原始参考图，由 Panel 155 独立单轮生成，并已清理 AI 元数据。横版严格为左侧原图、右侧设计，各占 50%；竖版严格为上方原图、下方设计，各占 50%。

示例局限：多张结果偏向建筑外景淡彩，且主体偏大，未完整达到一点透视室内重构与微小核心要求。为节省算力保留单次生成结果，不作为完整达标样板；实际任务仍须严格验收。
