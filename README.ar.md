...
# XXD Panel 155｜منظور النقطة الواحدة بألوان مخففة
...
تستخدم النماذج التالية مراجع أصلية مختلفة. أنشأ Panel 155 كل عمل بصورة مستقلة وفي جولة توليد واحدة، وقد أزيلت البيانات الوصفية للذكاء الاصطناعي. النماذج الأفقية تقسيمها صارم 50:50: الواقع يساراً والتصميم يميناً؛ والنماذج العمودية تقسيمها صارم 50:50: الواقع أعلى والتصميم أسفل.

حدود الأمثلة: تميل عدة نتائج إلى ألوان مخففة لواجهات معمارية بموضوعات كبيرة، ولا تحقق بالكامل إعادة بناء داخلية بمنظور نقطة واحدة ومحور صغير جداً. أُبقيت نتائج التوليد الواحد لتوفير الحوسبة، وليست نماذج امتثال كامل؛ يظل التحقق أثناء التنفيذ صارماً.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 155 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-155`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-155 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
